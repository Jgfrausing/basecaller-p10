...
torch.optim.RMSprop(lr, weight_decay)
...
def get_optimizer(config):
    if ADAM_W == config.optimizer:
        ...
        return partial(torch.optim.AdamW, amsgrad=True, lr=config.learning_rate, weight_decay=config.weight_decay, momentum=config.momentum)
    else:
