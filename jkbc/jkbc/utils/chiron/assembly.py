...
def simple_assembly_kernal(bpread, prev_bpread, error_rate, jump_step_ratio):
    ...
    log_px = dict()
    N = len(bpread)
    match_blocks = difflib.SequenceMatcher(
        a=bpread, b=prev_bpread).get_matching_blocks()
...
def global_alignment_kernal(bpread, prev_bpread):
    ...
    match = 1
    min_block_size = 3
    global_alignment = pairwise2.align.globalms(
        prev_bpread, bpread, match, mismatch, gap_open, gap_extend)
...
def simple_assembly(bpreads, jump_step_ratio, error_rate=0.2, kernal='global'):
    ...
    for indx, bpread in enumerate(bpreads):
        ...
        if kernal == 'simple':
            disp, log_p = simple_assembly_kernal(
                bpread, prev_bpread, error_rate, jump_step_ratio)
        elif kernal == 'global':
    ...
    for indx, bpread in enumerate(bpreads):
        ...
        if disp + pos + len(bpread) > census_len:
            ...
            concensus_qs = np.lib.pad(concensus_qs, ((0, 0), (0, 1000)),
                                      mode='constant', constant_values=0)
            census_len += 1000
        add_count_qs(concensus, concensus_qs, pos +
                     disp, bpread, qs_list[indx])
        pos += disp
        length = max(length, pos + len(bpread))
